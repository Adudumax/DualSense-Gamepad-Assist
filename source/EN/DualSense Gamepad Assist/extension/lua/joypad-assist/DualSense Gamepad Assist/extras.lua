...
M.update = function(vData, uiData, absInitialSteering, dt)
    ...
    if uiData.autoClutch then
        ...
        if vData.perfData.brokenEngineIni then
            if not errorShown1 then
                ac.setMessage("DualSense Gamepad Assist", "Error reading engine data. Automatic clutch and shifting will be disabled.")
                errorShown1 = true
            end
        elseif not canUseClutch then
            if not errorShown2 then
                ac.setMessage("DualSense Gamepad Assist", "Custom auto-clutch will not be used for this car.")
                errorShown2 = true
            end
        else
    ...
    if not vData.perfData.baseTorqueCurve then
        if not errorShown1 then
            ac.setMessage("DualSense Gamepad Assist", "Error reading engine data. Custom shifting modes will be disabled.")
            errorShown1 = true
        end
        return
    end
    ...
    if vData.vehicle.autoShift then
        if not errorShown3 then
            ac.setMessage("DualSense Gamepad Assist", "Disable AC's automatic shifting for the custom shifting modes to work!")
            errorShown3 = true
        end
        return
    end
    ...
    if vData.vehicle.mgukDeliveryCount > 0 and uiData.autoShiftingMode == 2 then
        if not errorShown4 then
            ac.setMessage("DualSense Gamepad Assist", "Automatic shifting will have reduced accuracy with this car.")
            errorShown4 = true
        end
    end
